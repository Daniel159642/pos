...
// --- STYLED COMPONENTS ---
const MockupContainer: React.FC<{ children: React.ReactNode, isError?: boolean, errorProgress?: number, resolveProgress?: number, overflowVisible?: boolean }> = ({ children, isError, errorProgress = 0, resolveProgress = 0, overflowVisible }) => {
    ...
    const activeGradColor2 = interpolateColors(resolveProgress, [0, 1], ["rgba(255, 255, 255, 0.4)", "rgba(255, 255, 255, 0.95)"]);
    const gradColor2 = interpolateColors(finalProgress, [0, 1], ["rgba(44, 25, 252, 0.1)", activeGradColor2]);

    const activeBorderColor = interpolateColors(resolveProgress, [0, 1], ["rgba(239, 68, 68, 0.4)", "rgba(255, 255, 255, 0.8)"]);
    const borderColor = interpolateColors(finalProgress, [0, 1], ["rgba(255, 255, 255, 0.45)", activeBorderColor]);
...
const AmbientGlow: React.FC<{ color: string, delay: number, x: string, y?: string, theme?: "dark" | "light" }> = ({ color, delay, x, y = "60%", theme = "dark" }) => {
    ...
    return (
        <div style={{
            position: "absolute",
            top: y, // Positioned more centrally for mobile
            left: x,
            width: "120%", // Covers side to side
            height: "50%", // More vertically constrained for better distribution
            background: `radial-gradient(ellipse at center, ${color} 0%, transparent 65%)`,
            borderRadius: "50%",
            filter: "blur(120px)", // Precise blur
...
const CinematicTitle: React.FC<{
    ...
    wordStagger?: number;
    fontSize?: number;
}> = ({ text, delay = 0, textColor = "#fff", suffixIcon, suffixIconDelay, wordStagger = 5, fontSize = 44 }) => {
    const frame = useCurrentFrame();
    const { fps } = useVideoConfig();
                    ...
                    return (
                        <span
                            ...
                            style={{
                                ...
                                marginLeft: -4 // Pull period closer
                            }}
                        >
                            {token}
                        </span>
...
const SceneWrapper: React.FC<{ children: React.ReactNode, title: string | React.ReactNode, subtitle?: string, theme?: "dark" | "light", titleDelay?: number, errorGlowFrame?: number, solveGlowFrame?: number, glowDelay?: number, whiteTransitionFrame?: number, errorRecoverFrame?: number, exitFrame?: number, glowSet?: "default" | "royal" | "white", blurAmount?: number, overlay?: React.ReactNode, zoomAmount?: number, cameraX?: number, cameraY?: number, cameraRotate?: number }> = ({ children, title, subtitle, theme = "dark", titleDelay = 0, errorGlowFrame, solveGlowFrame, glowDelay = 0, whiteTransitionFrame, errorRecoverFrame, exitFrame, glowSet = "default", blurAmount = 0, overlay, zoomAmount = 1, cameraX = 0, cameraY = 0, cameraRotate = 0 }) => {
    const frame = useCurrentFrame();
    ...
    const finalGlow1 = glowSet === "white" ? "#ffffff" : (glowSet === "royal" ? "#2c19fc" : interpolateColors(exitGlowProgress, [0, 1], [baseGlow1, "#2c19fc"]));
    const finalGlow2 = glowSet === "white" ? "#ffffff" : (glowSet === "royal" ? "#ffffff" : interpolateColors(exitGlowProgress, [0, 1], [baseGlow2, "#ffffff"]));
    const finalGlow3 = glowSet === "white" ? "#ffffff" : (glowSet === "royal" ? "#000000" : interpolateColors(exitGlowProgress, [0, 1], [baseGlow3, "#000000"]));
    const finalGlow4 = glowSet === "white" ? "#ffffff" : (glowSet === "royal" ? "#1a0fb3" : interpolateColors(exitGlowProgress, [0, 1], [baseGlow4, "#1a0fb3"]));
    ...
    const solveProgress = solveGlowFrame ? interpolate(frame, [solveGlowFrame, solveGlowFrame + 10], [0, 1], {
        extrapolateLeft: "clamp",
        extrapolateRight: "clamp",
        easing: Easing.inOut(Easing.ease)
    }) : 0;

    const errorToSolveFactor = solveProgress;

    const initialGlow1 = interpolateColors(errorProgress, [0, 1], [finalGlow1, interpolateColors(errorToSolveFactor, [0, 1], ["#ef4444", "#22c55e"])]); // Red to Green
    const initialGlow2 = interpolateColors(errorProgress, [0, 1], [finalGlow2, "#ffffff"]); // White
    const initialGlow3 = interpolateColors(errorProgress, [0, 1], [finalGlow3, interpolateColors(errorToSolveFactor, [0, 1], ["#fca5a5", "#bbf7d0"])]); // Light Red to Light Green
    const initialGlow4 = interpolateColors(errorProgress, [0, 1], [finalGlow4, interpolateColors(errorToSolveFactor, [0, 1], ["#dc2626", "#16a34a"])]); // Dark Red to Dark Green

    // Quick 3-frame transition to white if requested
    const whiteFactor = whiteTransitionFrame ? interpolate(frame, [whiteTransitionFrame, whiteTransitionFrame + 3], [0, 1], {
        extrapolateLeft: "clamp",
        extrapolateRight: "clamp"
    }) : 0;

    const glow1 = interpolateColors(whiteFactor, [0, 1], [initialGlow1, "#ffffff"]);
    const glow2 = interpolateColors(whiteFactor, [0, 1], [initialGlow2, "#ffffff"]);
    const glow3 = interpolateColors(whiteFactor, [0, 1], [initialGlow3, "#ffffff"]);
    const glow4 = interpolateColors(whiteFactor, [0, 1], [initialGlow4, "#ffffff"]);
    ...
    return (
        <AbsoluteFill style={{
            ...
            <AbsoluteFill style={{
                ...
                <AmbientGlow color={glow1} delay={glowDelay + 0} x="50%" y="25%" theme={theme} />
                <AmbientGlow color={glow2} delay={glowDelay + 4} x="30%" y="50%" theme={theme} />
                <AmbientGlow color={glow3} delay={glowDelay + 8} x="70%" y="75%" theme={theme} />
                <AmbientGlow color={glow4} delay={glowDelay + 12} x="50%" y="90%" theme={theme} />
...
const UploadScene = () => {
    ...
    const scale = interpolate(entrySpring, [0, 1], [1.1, 1.4]);
...
const ExtractionScene = () => {
    ...
    const resolveProgress = title4FlipProgress;
    const activeStatusColor = interpolateColors(resolveProgress, [0, 1], ["#ef4444", "#22c55e"]);
    ...
    const animatedSuffix = (
        <div style={{ position: "relative", width: 56, height: 56, marginTop: -8 }}>
            ...
            <div style={{ position: "absolute", inset: 0, transform: `scale(${sentSpring})`, opacity: sentSpring, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <Send size={48} color={activeStatusColor} strokeWidth={3} />
            </div>
        </div>
    );
    ...
    const customTitle = (
        <div style={{ transform: `scale(${sceneExitScale})`, opacity: sceneExitOpacity, display: "flex", justifyContent: "center", width: "100%" }}>
            <div style={{ position: "relative", height: 60, width: "100%", textAlign: "center", display: "flex", justifyContent: "center" }}>
                ...
                <div style={{ position: "absolute", width: "100%", left: 0, opacity: title3Opacity, transform: `perspective(600px) rotateX(${title3RotateX}deg)`, display: "flex", justifyContent: "center", gap: 10, alignItems: "baseline" }}>
                    <div style={{ position: "relative" }}>
    ...
    return (
        ...
        >
            <div style={{
                ...
                <MockupContainer errorProgress={titleFlipProgress} resolveProgress={resolveProgress}>
...
const InventoryScene = () => {
    ...
    return (
        <SceneWrapper title="Swftly restocks inventory." theme="light" exitFrame={dropStartFrame} whiteTransitionFrame={98}>
...
const AppDashboardScene = () => {
    ...
    const windowScale = interpolate(slideProgress, [0, 1], [0.45, 0.65]);
    ...
    const orbScale = interpolate(orbSpr, [0, 1], [0, 1.8]);
    const orbOpacity = interpolate(orbSpr, [0, 1], [0, 0.5]);
    ...
    const promoY = interpolate(promoSpr, [0, 1], [-200, 250]);
    ...
    const secondaryZoom = 1;
    const cameraY = 0;

    const blurStart = 25;
    const blurSpr = spring({
        frame: frame - blurStart,
        fps,
        config: { damping: 20, stiffness: 60 }
    });
    const blurAmount = interpolate(blurSpr, [0, 1], [0, 10]);
    ...
    const cursorY = interpolate(cursorProgress, [0, 1], [100, -620]);
        ...
        <div style={{
            ...
            <div style={{
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 4, flex: 1 }}>
                    <div style={{ fontSize: 32, fontWeight: 900, color: "#000", letterSpacing: "-0.5px" }}>Summer Flash Sale ☀️</div>
                    <div style={{ fontSize: 22, fontWeight: 600, color: "#374151", opacity: 0.9 }}>Select categories are up to 50% off!</div>
                </div>
            </div>
        </div>
    );
        ...
        >
            <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", pointerEvents: "none" }}>
                <div style={{
                    ...
                    height: 600,
                    borderRadius: "50%",
                    background: `radial-gradient(circle, ${BLUE} 0%, rgba(10, 70, 245, 0.6) 40%, transparent 75%)`,
                    transform: `scale(${orbScale})`,
                    opacity: orbOpacity * 1.2,
                    filter: "blur(80px)",
                    zIndex: 0
                }} />
            </div>
                                ...
                                {[
                                    ...
                                    return (
                                        <div key={idx} style={{
                                            ...
                                            <Img src={staticFile(`/${notif.icon}`)} style={{ width: 48, height: 48, objectFit: "contain" }} />
                                            <div style={{ display: "flex", flexDirection: "column", gap: 2, minWidth: 0 }}>
                                                <div style={{ fontSize: 18, fontWeight: 800, color: "#111", textTransform: "uppercase", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{notif.title}</div>
                                                <div style={{ fontSize: 14, fontWeight: 500, color: "rgba(107, 114, 128, 0.8)" }}>{notif.subtitle}</div>
                                            </div>
                                        </div>
                                    );
                                })}
...
const MarketingTaglineScene = () => {
    ...
    const successGlowMain = interpolateColors(successSpr, [0, 1], [orbMain, "rgba(34, 197, 94, 0.9)"]);
    const successGlowOuter = interpolateColors(successSpr, [0, 1], [orbOuter, "rgba(34, 197, 94, 0.4)"]);
        ...
        <AbsoluteFill style={{
            ...
            <AbsoluteFill style={{
                ...
                <div style={{
                    ...
                    width: 300,
                    height: 300,
                    background: `radial-gradient(circle, ${successGlowMain} 0%, ${successGlowOuter} 50%, transparent 70%)`,
                ...
                <div style={{
                    ...
                    {words1.map((word, i) => {
                        ...
                        if (isPeriod) {
                            ...
                            return (
                                <div key={i} style={{
                                    fontSize: "56px", fontWeight: 400, color: "#000", fontFamily: "EB Garamond, serif",
                        ...
                        const width = interpolate(spr, [0, 1], [0, [150, 95, 105, 235][i]]);

                        return (
                            <div key={i} style={{ overflow: "hidden", width, display: "flex", justifyContent: "flex-start", paddingLeft: isFirst ? "15px" : 0, opacity }}>
                                <div style={{
                                    transform: `translateY(${translateY}px) scale(${scale})`,
                                    fontSize: "56px", fontWeight: 400, color: currentColor, fontFamily: "EB Garamond, serif",
                ...
                <div style={{
                    ...
                    {words2.map((word, i) => {
                        ...
                        const containerWidth = isSMS ? interpolate(flip2Spr, [0, 1], [100, 125]) : "auto";
                        return (
                ...
                {(() => {
                    ...
                    const cursorX = interpolate(cursorSpr, [0, 1], [1100, targetX]);
                    const cursorY = interpolate(cursorSpr, [0, 1], [800, clickTargetY]);
                ...
                {(() => {
                    ...
                    const popX = interpolate(popSpr, [0, 1], [targetX - 540, 0]);
                    const popY = interpolate(popSpr, [0, 1], [clickTargetY - 960, 0]);
    ...
    return (
        <AbsoluteFill style={{
            ...
            {(() => {
                ...
                const orbScale = interpolate(orbSpr, [0, 1], [0.5, 2.5]);
                const orbOpacity = interpolate(orbSpr, [0, 1], [0, 0.45]);
                ...
                return (
                    <div style={{
                        ...
                        width: 400,
                        height: 400,
                        background: "radial-gradient(circle, #ffffff 0%, rgba(255,255,255,0.7) 40%, transparent 75%)",
            ...
            <div style={{ zIndex: 10, position: "absolute", top: "60%", width: "100%", display: "flex", justifyContent: "center" }}>
                <CinematicTitle text="Swftly does it all." textColor="#fff" delay={10} fontSize={64} />
            </div>
