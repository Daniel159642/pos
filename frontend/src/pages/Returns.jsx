...
function Returns() {
  ...
  const loadOrders = async () => {
    ...
    try {
      ...
      // Show recent orders (last 100 for better coverage)
      if (result.data) {
      ...
      setToast({ message: 'Error loading orders. Please refresh the page.', type: 'error' })
    } finally {
  ...
  const handleBarcodeScan = async (barcode) => {
    try {
      ...
      if (matchingOrder) {
        ...
        // Add order to ordersData if not already present
        if (ordersData && ordersData.data) {
          const orderExists = ordersData.data.some(o => o.order_id === matchingOrder.order_id)
          if (!orderExists) {
  ...
  const fetchOrderById = async (orderId) => {
    ...
    try {
      ...
      let foundOrder = null
      if (ordersData && ordersData.data) {
        foundOrder = ordersData.data.find(o => o.order_id === parseInt(orderId))
      }
      ...
      if (!foundOrder) {
        ...
        if (searchResult.data && searchResult.data.length > 0) {
          ...
          if (ordersData && ordersData.data) {
            const orderExists = ordersData.data.some(o => o.order_id === foundOrder.order_id)
            if (!orderExists) {
              setOrdersData(prev => ({
                ...prev,
  ...
  const searchOrder = async () => {
    if (!orderNumber.trim()) {
      setToast({ message: 'Please enter an order number', type: 'error' })
      return
    }
    ...
    try {
      ...
      setToast({ message: 'Error searching for order', type: 'error' })
      console.error(err)
    } finally {
      setLoading(false)
    }
  ...
  let filteredByOrder = filteredBySearch
  if (scannedOrderId) {
    filteredByOrder = filteredBySearch.filter(row => {
  ...
  const handleRowClick = async (row) => {
    ...
    try {
      ...
      console.log(`All order items fetched:`, items.length)
      console.log(`Looking for order_id:`, normalizedOrderId, typeof normalizedOrderId)
      
      // Filter items for this order - ensure both are compared as numbers
      let orderItems = items.filter(item => {
        const itemOrderId = parseInt(item.order_id || item.orderId || 0)
        const matches = itemOrderId === normalizedOrderId
        if (matches) {
      ...
      console.log(`Order ${normalizedOrderId} items found:`, orderItems.length, orderItems)
      
      // If no items found, try fetching directly by order_id
      if (orderItems.length === 0) {
        console.log('No items found in all items, trying direct fetch by order_id...')
        try {
          const directResponse = await fetch(`/api/order_items?order_id=${normalizedOrderId}`)
          const directResult = await directResponse.json()
          console.log('Direct fetch result:', directResult)
          if (directResult.data && directResult.data.length > 0) {
            console.log('Found items via direct fetch:', directResult.data.length, directResult.data)
            // Use the directly fetched items
            orderItems = directResult.data
          }
        } catch (directErr) {
      ...
      setToast({ message: 'Error loading order details', type: 'error' })
    } finally {
  ...
  return (
    <div style={{ 
      ...
      <div style={{ marginBottom: '20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
              <div style={{ 
                ...
                >
                  ...
                  {scannedOrderId && (
                    <div 
                      ...
                      <span>
                        {ordersData?.data?.find(o => o.order_id === scannedOrderId)?.order_number || `Order #${scannedOrderId}`}
                      </span>
                      <button
                        onClick={(e) => {
                          e.stopPropagation()
                ...
                <input
                  type="text"
                  placeholder={(scannedOrderId || scannedProducts.length > 0) ? "" : "Search orders..."}
                  value={searchQuery}
      ...
      {/* View Toggle Buttons */}
      <div style={{ display: 'flex', justifyContent: 'flex-start', alignItems: 'center', marginBottom: '20px', gap: '8px' }}>
        <button
                                          ...
                                          <tbody>
                                            {details.items.map((item, itemIdx) => (
                                              <tr key={item.order_item_id || itemIdx} style={{ backgroundColor: itemIdx % 2 === 0 ? (isDarkMode ? 'var(--bg-primary, #1a1a1a)' : '#fff') : (isDarkMode ? 'var(--bg-secondary, #2d2d2d)' : '#f8f9fa') }}>
                                                ...
                                                <td style={{ padding: '8px', textAlign: 'right', fontSize: '13px', color: isDarkMode ? 'var(--text-primary, #fff)' : '#333' }}>${parseFloat(item.unit_price || 0).toFixed(2)}</td>
                                                <td style={{ padding: '8px', textAlign: 'right', fontSize: '13px', color: isDarkMode ? 'var(--text-primary, #fff)' : '#333' }}>${parseFloat(item.discount || 0).toFixed(2)}</td>
                                                <td style={{ padding: '8px', textAlign: 'right', fontSize: '13px', color: isDarkMode ? 'var(--text-primary, #fff)' : '#333' }}>${parseFloat(item.subtotal || 0).toFixed(2)}</td>
                                              </tr>
                                            ))}
                                          </tbody>
                                        </table>
          ...
          {order && (
            <div style={{
              ...
              <div style={{
                ...
                {/* Return Type Selection - First Question */}
                  ...
                  <FormField>
                    ...
                    <div ref={returnTypeDropdownRef} style={{ position: 'relative', width: '100%' }}>
                      <button
                        ...
                        onBlur={(e) => {
                          // Don't blur if clicking on dropdown items
                          setTimeout(() => {
                        ...
                        <div style={{
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              setExchangeTiming('now') // Exchange is used now
                              setReturnTypeDropdownOpen(false)
                            }}
                            style={{
                              width: '100%',
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              setExchangeTiming('later') // Store credit is used later
                              setReturnTypeDropdownOpen(false)
                            }}
                            style={{
                              width: '100%',
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              setExchangeTiming(null) // Refund doesn't need exchange timing
                              setReturnTypeDropdownOpen(false)
                            }}
                            style={{
                              width: '100%',
                ...
                {orderItems.length > 0 && (
                  <div style={{ marginBottom: '24px' }}>
                    ...
                    <div style={{ marginTop: '20px' }}>
                      ...
                      {/* Return Amount Summary - Always visible at bottom */}
                      {(() => {
                        let returnSubtotal = 0
                        let returnTax = 0
                        let returnProcessingFee = 0
                        ...
                        // Calculate processing fee (if original order had one, apply proportional fee)
                        if (order.transaction_fee && order.subtotal) {
                          const feeRate = parseFloat(order.transaction_fee) / parseFloat(order.subtotal)
                          returnProcessingFee = returnSubtotal * feeRate
                        }
                      ...
                      <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
                        ...
                        <button
                          ...
                          disabled={loading || Object.keys(selectedItems).length === 0 || !returnType}
                          style={{
                            padding: '4px 16px',
                            height: '28px',
                            display: 'flex',
                            ...
                            justifyContent: 'center',
                            whiteSpace: 'nowrap',
                            backgroundColor: loading || Object.keys(selectedItems).length === 0 || !returnType || (returnType === 'exchange' && !exchangeTiming) ? '#ccc' : `rgba(${themeColorRgb}, 0.7)`,
                            border: `1px solid rgba(${themeColorRgb}, 0.5)`,
                            borderRadius: '8px',
                            fontSize: '14px',
                            fontWeight: 600,
                            color: '#fff',
                            cursor: loading || Object.keys(selectedItems).length === 0 || !returnType || (returnType === 'exchange' && !exchangeTiming) ? 'not-allowed' : 'pointer',
                            transition: 'all 0.3s ease',
                            boxShadow: `0 4px 15px rgba(${themeColorRgb}, 0.3)`
                          }}
                        >
